...
<%--
<c:set />
1.4가지 영역의 속성(attribute)를 생성 또는 변경 또는 삭제
...
<c:set var="myAttr1" value="hello1" /> <%--scop기본값은 page --%>
...
<c:set var="myAttr5">Hello World, value5!!!</c:set>
<p>${myAttr5 }</p>

<c:set var="myAttr5">Hello JAVA, value5~~~~~</c:set>
<p>${myAttr5 }</p>
...
<p>${myAttr6.name }, ${myAttr6.email }, ${myAttr6.ssn}, ${myAttr6.address }</p>

<%--value가 null이면 attr이 삭제됨--%> 
<c:set var="myAttr6" value="${member2 }" />
<p>myAttr6에 null값 set후: ${myAttr6.name }, ${myAttr6.email }, ${myAttr6.address }</p>

<%--지우는 태그도 있음. scope은 page가 기본값임--%>
<c:remove var="myAttr5" scope="page" />
<p>myAttr5 삭제 후 : ${myAttr5 }</p>
